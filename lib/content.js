// Auto-generated from the site's HTML pages. Each entry provides the
// Markdown representation served when a request negotiates
// `Accept: text/markdown` on the matching route (see /middleware.js).
// Regenerate by re-running the extraction script if page copy changes.
export const PAGES = {
    "/": {
          title: `Vivid Partners · Private Venture Holding Company | Toronto`,
          markdown: `---
          title: Vivid Partners · Private Venture Holding Company | Toronto
          url: https://www.vividpartners.ca/
          description: Vivid Partners is a leading private venture holding company based in Toronto, dedicated to acquiring, strengthening, and building exceptional businesses for the long term. We believe enduring companies are built through patient ownership, disciplined execution, and trusted partnerships.
          ---

          # Vivid Partners

          Vivid Partners is a private venture holding company that backs exceptional founders and invests in businesses with long‑term potential. Our role extends beyond capital, as we work alongside our portfolio companies to help them scale and create lasting value.

          [Let's Talk](/contact)

          Where We're Building

          TORONTO
          LONDON
          SAN FRANCISCO
          MIAMI
          NEW YORK

          A disciplined approach to helping businesses grow by investing in their most valuable asset:

          their people.

          Who We Are

          ## We are Vivid Partners.

          We've helped back multiple companies, guiding them as they scale across international markets.

          [Learn More](/team)

          Portfolio

          ## Partnering with Entrepreneurs

          We back founders who've already proven the model, then help them scale it.

          [View Portfolio](/portfolio)

          ![Vivid Agency logo](/vivid-agency-logo.webp)

          Vivid Agency

          A Toronto-based consulting practice, acquired by Vivid Partners, with the same team continuing to run it.

          Status

          Acquired

          Location

          Toronto, Ontario

          [Read More +](/portfolio#vivid-agency)

          ![Kora logo](/kora-logo.webp)

          Kora

          An all-in-one CRM and marketing automation platform, acquired by Vivid Partners to invest in its growth.

          Status

          Acquired

          Location

          London, United Kingdom

          [Read More +](/portfolio#kora)

          ![Keyflow logo](/keyflow-logo.webp)

          Keyflow

          An early-stage AI startup based in San Francisco. Vivid Partners provided early-stage capital.

          Status

          Active Partnership

          Location

          San Francisco, California

          [Read More +](/portfolio#keyflow)

          ## FAQs

          1. What can I expect from the partnership process?

          Every partnership starts with a conversation, not a term sheet. We take the time to understand your business, your goals, and what a good outcome looks like for you before we talk structure.

          2. What do your deal structures look like?

          We work across majority and minority positions, growth equity, and private credit, structured around what makes sense for the business rather than a one-size-fits-all template.

          3. What is the value of my business?

          Valuation depends on growth trajectory, margins, and market position. We'll walk you through our thinking directly and transparently rather than leaving you guessing.

          4. What happens to my role after we partner?

          Most founders stay on and keep running the business day to day. We're there for strategic support and capital, not to replace the people who built it.

          5. What happens to my team?

          Your team is part of what we're investing in. We back businesses with the intent to grow them, and that means growing the people already there.

          6. What happens to my brand?

          In most cases your brand stays exactly as it is. We back what's already working rather than folding it into something else.

          7. What sectors do you focus on?

          We concentrate on growth equity, private credit, and real assets, with a preference for founder-led businesses with proven fundamentals.

          8. What does "Partners" mean in "Vivid Partners"?

          It's how we work: alongside founders, for the long term, not just as a source of capital that shows up once a quarter.
          `,
    },
    "/team": {
          title: `Team · Vivid Partners`,
          markdown: `---
          title: Team · Vivid Partners
          url: https://www.vividpartners.ca/team
          description: Meet the team behind Vivid Partners: co-founders Mithun Narendran and Abishan Ravinthran, COO Vathusan Udayakumaran, and equity research analyst Rathusan Udayakumaran.
          ---

          # Our People

          Our team draws on backgrounds in M&A, entrepreneurship, and business leadership. Vivid Partners is further backed by a network of external investors and advisors.

          [MN

          ### Mithun Narendran

          Co-Founder](/team-mithun)
          [AR

          ### Abishan Ravinthran

          Co-Founder](/team-abishan)
          [VU

          ### Vathusan Udayakumaran

          Chief Operating Officer](/team-vathusan)
          [RU

          ### Rathusan Udayakumaran

          Equity Research Analyst](/team-rathusan)
          `,
    },
    "/portfolio": {
          title: `Portfolio · Vivid Partners`,
          markdown: `---
          title: Portfolio · Vivid Partners
          url: https://www.vividpartners.ca/portfolio
          description: The Vivid Partners portfolio: Vivid Agency, Kora, and Keyflow. Businesses we've acquired and back with patient, long-term capital.
          ---

          [Back to Vivid Partners](/)
          Portfolio

          # Companies we've partnered with

          Two acquisitions and one early-stage investment, each an active part of the Vivid Partners portfolio.

          Status

          Acquired

          Location

          Toronto, Ontario

          Sector

          Consulting, Web Dev & Paid Ads

          ### Vivid Agency

          Vivid Agency is a Toronto-based consulting practice covering web development, paid media, SEO, social media management, and AI-driven workflows. Vivid Partners acquired Vivid Agency, with the same team continuing to run it day to day.

          Status

          Acquired

          Location

          London, United Kingdom

          Sector

          CRM & Marketing Automation

          ### Kora

          Kora is an all-in-one CRM and marketing automation platform, built to handle pipeline management, client communication, and campaign automation in a single system. Vivid Partners acquired Kora to invest further in its product and growth.

          Status

          Active Partnership

          Location

          San Francisco, California

          Sector

          Artificial Intelligence

          ### Keyflow

          Keyflow is an early-stage AI startup based in San Francisco. Vivid Partners provided early-stage capital to support product development and go-to-market.
          `,
    },
    "/partner-stories": {
          title: `Partner Stories · Vivid Partners`,
          markdown: `---
          title: Partner Stories · Vivid Partners
          url: https://www.vividpartners.ca/partner-stories
          description: Stories from the founders and operators who have partnered with Vivid Partners: what selling to a long-term holding company actually looks like.
          ---

          [Back to Vivid Partners](/)
          Partner Stories

          # Stories from our partners

          Two acquisitions and one early-stage investment. Here's the story behind each.

          Consulting, Web Dev & Paid Ads · Toronto, Ontario

          ### Bringing Our Own Agency In-House

          Vivid Agency is a Toronto-based consulting practice covering web development, paid media, SEO, social media management, and AI-driven workflows. Vivid Partners acquired Vivid Agency, keeping the same team in place to run it day to day.

          How did this come together?

          The Vivid Partners team already ran Vivid Agency before the firm existed. Bringing it into the portfolio formalized a relationship that was already in place.

          What changed after the acquisition?

          Day-to-day operations stayed the same. Vivid Partners now provides capital and strategic support behind the agency's growth.

          Where things stand

          Vivid Agency continues to operate under its founding team, serving clients across web development, paid media, and more.

          #### Looking Ahead

          Vivid Partners plans to keep investing in Vivid Agency's growth as it takes on new clients and expands its service offerings.

          CRM & Marketing Automation · London, United Kingdom

          ### Building a Better Way to Run a Business

          Kora is an all-in-one CRM and marketing automation platform, built to handle pipeline management, client communication, and campaign automation in a single system. Vivid Partners acquired Kora to invest further in its product and growth.

          What does Kora do?

          Kora brings CRM, marketing automation, and client communication into a single platform, built to be faster and more intuitive than the tools most small businesses are stuck using today.

          Why did Vivid Partners acquire it?

          Kora solved a problem the team saw firsthand running other businesses: most CRM and automation platforms are clunky and overbuilt. Kora is built to be simpler and more effective.

          Where things stand

          Kora is an active part of the Vivid Partners portfolio, with ongoing product development.

          #### Looking Ahead

          Vivid Partners is investing in Kora's product and go-to-market as it grows its customer base.

          Artificial Intelligence · San Francisco, California

          ### Backing AI Outside Our Own Walls

          Keyflow is an early-stage AI startup based in San Francisco. It's Vivid Partners' first investment outside of the companies it directly acquired and operates.

          Why did Vivid Partners invest?

          Keyflow represented a chance to back a team building something genuinely new in AI, outside the businesses Vivid Partners runs directly.

          What does Vivid Partners provide?

          Early-stage capital to support product development and go-to-market.

          Where things stand

          Keyflow is an active, ongoing investment for Vivid Partners.

          #### Looking Ahead

          Vivid Partners continues to support Keyflow as it builds out its product and customer base.
          `,
    },
    "/careers": {
          title: `Careers · Vivid Partners`,
          markdown: `---
          title: Careers · Vivid Partners
          url: https://www.vividpartners.ca/careers
          description: Careers at Vivid Partners, a Toronto-based private venture holding company. Join a team building enduring businesses for the long term.
          ---

          [Back to Vivid Partners](/)
          Careers

          # Building the team, one conversation at a time

          We're small on purpose. If you want to learn the work early and don't need a formal posting to reach out, start here.

          Vivid Partners is a small, family-backed firm. Right now that means a handful of people, not a floor of analysts. We're not running a formal recruiting cycle, but we're always glad to hear from people who want to learn the work early.

          We're most interested in people who are curious about how small businesses actually run, not just how they're valued on paper. If that's you, an introduction is a better first step than a resume.

          - No open roles are posted right now. We hire when a specific need comes up, not on a schedule.
          - Analyst and internship conversations are welcome from students and early-career candidates.
          - Operators from our portfolio companies' industries are always worth a conversation, whether or not there's a role.

          ### Start a conversation

          Send a short note about what you're working on now and what you'd want to learn here. No formal application needed.

          [Email the team](mailto:info@vividpartners.ca?subject=Careers%20at%20Vivid%20Partners)
          `,
    },
    "/resources": {
          title: `Resources · Vivid Partners`,
          markdown: `---
          title: Resources · Vivid Partners
          url: https://www.vividpartners.ca/resources
          description: Guides and resources from Vivid Partners on selling a business, valuation, and long-term ownership.
          ---

          [Back to Vivid Partners](/)
          Resources

          # Notes from the partnership desk

          Short, practical write-ups on how we think about structure, growth, and working with founders. Not a polished investor newsletter, just what comes up in conversations.

          Insights · Selling

          ### [How to sell your business in Ontario: a founder's guide](/selling-your-business-in-ontario)

          The whole process in plain language: preparing your numbers, finding buyers, valuation, due diligence, and closing. [Read the guide →](/selling-your-business-in-ontario)

          Insights · Fundamentals

          ### [What is a private venture holding company?](/what-is-a-holding-company)

          How the model differs from a VC fund or private equity firm, and why founders choose to sell to one. [Read more →](/what-is-a-holding-company)

          Insights · Selling

          ### [Selling to a holding company vs. private equity](/holding-company-vs-private-equity)

          Deal structure, debt, timelines, and what happens to your team: how the two paths actually differ once the deal closes. [Read more →](/holding-company-vs-private-equity)

          Insights · Valuation

          ### [How small businesses are actually valued](/how-small-businesses-are-valued)

          SDE, EBITDA, multiples, and the factors that actually move them, explained without the jargon. [Read more →](/how-small-businesses-are-valued)

          Insights · Succession

          ### [Succession planning for Canadian business owners](/succession-planning-canada)

          Family transfer, management buyout, or sale: the real options and the timeline each one needs. [Read more →](/succession-planning-canada)

          Notes · Partnership

          ### What we actually mean by "partnership"

          The word gets used loosely in this industry, so it's worth being specific about what it means at Vivid. We're not a source of capital that shows up once a quarter for a board update. We're closer to a second set of hands: available for a call about a hiring decision, a lease negotiation, or a difficult customer, not just for the numbers.

          That only works at our size because we're deliberately not spread across dozens of companies. Every founder we back can reach the same one or two people who did the diligence in the first place.

          Notes · Deal Structure

          ### Growth equity vs. private credit, in plain terms

          Growth equity means Vivid takes a stake in the business and shares in the upside alongside the founder. It makes sense when a company needs capital to expand and the owner is comfortable giving up some ownership to grow faster.

          Private credit means Vivid lends against the business instead of buying into it. It tends to fit owners who want capital for a specific purpose, like equipment, inventory, or an acquisition, without changing who owns the company. We're upfront early in a conversation about which structure actually fits, rather than defaulting to whichever one we'd prefer.

          Notes · Founders

          ### Why we look for founder-led businesses

          A founder who's already proven a model has done the hardest part: figured out what customers actually want to pay for. Our job is narrower than that. We help them do more of what's already working, with fewer of the operational bottlenecks that come from growing without extra capital or a second opinion.

          It also means the businesses we back tend to keep their culture and their customer relationships intact through the partnership, since the person who built those relationships is still running the business day to day.
          `,
    },
    "/contact": {
          title: `Contact · Vivid Partners`,
          markdown: `---
          title: Contact · Vivid Partners
          url: https://www.vividpartners.ca/contact
          description: Contact Vivid Partners in Toronto. Considering selling your business or partnering with us? Reach out at info@vividpartners.ca.
          ---

          [Back to Vivid Partners](/)
          Contact

          # Let's talk

          Tell us a bit about your business or what you're working on. A real person on the team reads every message.

          ## Let's talk

          Whether you're a founder weighing a partnership, an advisor with a referral, or a portfolio company operator, the fastest way to reach us is directly.

          #### Email

          [info@vividpartners.ca](mailto:info@vividpartners.ca)

          #### Phone

          [+1 (647) 313-4635](tel:+16473134635)

          #### Office

          150 King St W, Toronto, ON
          `,
    },
    "/team-mithun": {
          title: `Mithun Narendran · Co-Founder · Vivid Partners`,
          markdown: `---
          title: Mithun Narendran · Co-Founder · Vivid Partners
          url: https://www.vividpartners.ca/team-mithun
          description: Mithun Narendran is Co-Founder of Vivid Partners, leading the firm's investment strategy and capital allocation decisions.
          ---

          MN

          # Mithun Narendran

          ---

          Role

          Co-Founder

          Location

          Toronto

          Links

          Mithun co-founded Vivid Partners and leads deal origination, acquisitions, and firm strategy.

          He previously directed deal execution and client acquisition at Vivid Agency, closing the majority of the firm's new business.

          [BACK TO TEAM](/team)
          `,
    },
    "/team-abishan": {
          title: `Abishan Ravinthran · Co-Founder · Vivid Partners`,
          markdown: `---
          title: Abishan Ravinthran · Co-Founder · Vivid Partners
          url: https://www.vividpartners.ca/team-abishan
          description: Abishan Ravinthran is Co-Founder of Vivid Partners, overseeing business development and partner relationships.
          ---

          AR

          # Abishan Ravinthran

          ---

          Role

          Co-Founder

          Location

          Toronto

          Links

          Abishan co-founded Vivid Partners and leads business development, pitching, and deal origination.

          He previously led new business development at Vivid Agency, working closely with Mithun to source and close new engagements.

          #### Education

          University of Toronto / Honours Bachelor of Commerce, Finance and Economics (Candidate).

          [BACK TO TEAM](/team)
          `,
    },
    "/team-vathusan": {
          title: `Vathusan Udayakumaran · COO · Vivid Partners`,
          markdown: `---
          title: Vathusan Udayakumaran · COO · Vivid Partners
          url: https://www.vividpartners.ca/team-vathusan
          description: Vathusan Udayakumaran is Chief Operating Officer of Vivid Partners, managing day-to-day operations and client relations.
          ---

          VU

          # Vathusan Udayakumaran

          ---

          Role

          Chief Operating Officer

          Location

          Toronto

          Links

          Vathusan oversees firm operations at Vivid Partners, including deal logistics, infrastructure, and internal process.

          He previously led operations at Vivid Agency.

          #### Education

          Toronto Metropolitan University / Bachelor of Engineering – Mechanical Engineering (Candidate).

          [BACK TO TEAM](/team)
          `,
    },
    "/team-rathusan": {
          title: `Rathusan Udayakumaran · Equity Research Analyst · Vivid Partners`,
          markdown: `---
          title: Rathusan Udayakumaran · Equity Research Analyst · Vivid Partners
          url: https://www.vividpartners.ca/team-rathusan
          description: Rathusan Udayakumaran is Equity Research Analyst at Vivid Partners, conducting equity research and financial analysis.
          ---

          RU

          # Rathusan Udayakumaran

          ---

          Role

          Equity Research Analyst

          Location

          Toronto

          Links

          Rathusan leads equity research at Vivid Partners, covering prospective investments and monitoring portfolio performance.

          He previously led research and analysis at Vivid Agency.

          #### Education

          Toronto Metropolitan University / Honours Bachelor of Commerce, Business Management and Real Estate Management (Candidate).

          [BACK TO TEAM](/team)
          `,
    },
    "/privacy": {
          title: `Privacy Policy · Vivid Partners`,
          markdown: `---
          title: Privacy Policy · Vivid Partners
          url: https://www.vividpartners.ca/privacy
          description: How Vivid Partners collects, uses, and protects your information.
          ---

          [Back to Vivid Partners](/)
          Legal

          # Privacy Policy

          How Vivid Partners handles information submitted through this website.

          Last updated July 2026

          ## 1. What we collect

          When you use the contact form, newsletter signup, or partner login on this site, we collect the information you provide directly, such as your name, email address, company, and message.

          ## 2. How we use it

          - To respond to inquiries submitted through the contact form.
          - To send newsletter updates to people who sign up for them.
          - To manage access to the partner portal for existing partners and portfolio companies.

          We do not sell or rent your information to third parties.

          ## 3. Cookies and analytics

          This site may use basic analytics to understand how visitors use it. This does not include selling personal data to advertisers.

          ## 4. Your choices

          You can unsubscribe from the newsletter at any time using the link in any email, or by contacting us directly. You can also ask us to delete information you've submitted through the contact form.

          ## 5. Contact

          Questions about this policy can be sent to [info@vividpartners.ca](mailto:info@vividpartners.ca).
          `,
    },
    "/terms": {
          title: `Terms of Use · Vivid Partners`,
          markdown: `---
          title: Terms of Use · Vivid Partners
          url: https://www.vividpartners.ca/terms
          description: Terms of use for the Vivid Partners website.
          ---

          [Back to Vivid Partners](/)
          Legal

          # Terms of Use

          The rules for using this website. This page is general in nature and isn't a substitute for legal advice.

          Last updated July 2026

          ## 1. Acceptance of terms

          By accessing this website, you agree to these Terms of Use. If you do not agree, please do not use this site.

          ## 2. Not an offer or advice

          Nothing on this website constitutes an offer to sell, or a solicitation of an offer to buy, any security or investment product, and nothing here should be treated as financial, legal, or tax advice. Any information about Vivid Partners' portfolio, approach, or past results is provided for general informational purposes only.

          ## 3. No guarantee of outcomes

          References to specific companies, results, or figures on this site describe individual partnerships and are not representative of future performance. Past results do not guarantee future outcomes.

          ## 4. Intellectual property

          The content, layout, and design of this website belong to Vivid Partners unless otherwise noted, and may not be reproduced without permission.

          ## 5. Changes to these terms

          We may update these terms from time to time. Continued use of the site after changes are posted means you accept the updated terms.

          ## 6. Contact

          Questions about these terms can be sent to [info@vividpartners.ca](mailto:info@vividpartners.ca).
          `,
    },
    "/selling-your-business-in-ontario": {
          title: `How to Sell Your Business in Ontario: A Founder's Guide · Vivid Partners`,
          markdown: `---
          title: How to Sell Your Business in Ontario: A Founder's Guide · Vivid Partners
          url: https://www.vividpartners.ca/selling-your-business-in-ontario
description: What actually happens when you sell a business in Ontario: preparing your numbers, finding buyers, valuation, due diligence, and closing, explained plainly by Vivid Partners.
---

[Back to Resources](/resources)
Insights · Selling

# How to Sell Your Business in Ontario: A Founder's Guide

The process is less mysterious than it looks from the outside. Here is the whole arc, from first thought to closing day, in plain language.

Most founders sell a business exactly once. The buyer across the table has usually done it many times. That gap (not valuation math, not lawyers) is what costs sellers the most money. This guide walks through the whole process the way it actually unfolds in Ontario, so the first conversation you have with a buyer isn't also the first time you're hearing how any of this works.

## Start before you're ready to sell

The best exits are set up one to three years before a sale process starts. Buyers pay for clean, believable numbers, and they discount everything else. If your books mix personal expenses with business ones, if revenue lives in spreadsheets instead of accounting software, or if the company's biggest customer relationship exists only in your head, fix those things now. None of it requires a banker: it requires tidy books, customer contracts in writing, and at least one person besides you who knows how the business runs.

## Know what you're actually selling

In Canada, a sale is structured as either a share sale or an asset sale. Sellers usually prefer share sales because of the Lifetime Capital Gains Exemption, which can shelter a substantial amount of the gain on qualified small business corporation shares from tax. Buyers often prefer asset sales because they cherry-pick what they take on. Where a deal lands between those two is a negotiation, and it changes your after-tax outcome enormously. This is the single best reason to involve an accountant early, before you've agreed on a headline price.

## Where buyers actually come from

Ontario sellers generally find buyers through four channels: competitors and strategic acquirers who already know the business, business brokers and M&A advisors who run a process for a fee, online marketplaces, and direct approaches from investment firms and holding companies. Inbound interest is flattering, but a single-buyer negotiation is the weakest position you can sell from. Even two credible interested parties changes the dynamic completely.

## How the price gets set

Most small and mid-sized businesses in Canada trade on a multiple of SDE (seller's discretionary earnings) or EBITDA. The multiple moves with how transferable the business is: recurring revenue, a management team that stays, low customer concentration, and documented processes all push it up. A business that stops working the day the founder leaves gets the low end no matter how profitable it is. If you want the deeper mechanics, we wrote a separate piece on [how small businesses are actually valued](/how-small-businesses-are-valued).

## The offer is not the deal

A letter of intent (LOI) sets the headline price and grants the buyer exclusivity to do due diligence. Expect diligence to take six to twelve weeks: financial records, tax filings, contracts, employees, litigation, systems. Two things protect you here. First, disclose problems early: diligence exists to find them, and a surprise found late becomes a price cut. Second, watch the structure as closely as the number: how much is cash at closing versus earn-outs, vendor take-back notes, or holdbacks. A lower price paid fully in cash is often better than a bigger headline paid slowly and conditionally.

## Closing, and what nobody tells you

Closing itself is anticlimactic: signatures, funds flow, done. What surprises founders is what comes after: most deals include a transition period where you stay on for months, sometimes a year or two, and many include some form of ongoing tie to the business's performance. Be honest with yourself about how long you're willing to stay and in what role, and negotiate that explicitly rather than letting it default.

## Choosing who to sell to

Price matters, but so does what happens to the thing you built. Strategic buyers often fold the company into their own operations; financial buyers vary from quick-flip models to long-term holds. Ask any buyer three questions: what happened to the last three businesses you bought, who from those teams is still there, and what do you plan to change in year one. The answers tell you more than the pitch deck does.

This article is general information, not legal, tax, or financial advice. Talk to your lawyer and accountant before signing anything.

**Thinking about your own next step?** Vivid Partners acquires and invests in founder-led businesses across the Greater Toronto Area and beyond. A conversation costs nothing and stays confidential.

[Talk to us](/contact)
`,
    },
    "/what-is-a-holding-company": {
          title: `What Is a Private Venture Holding Company? · Vivid Partners`,
          markdown: `---
          title: What Is a Private Venture Holding Company? · Vivid Partners
          url: https://www.vividpartners.ca/what-is-a-holding-company
          description: A plain-language explainer of what a private venture holding company does, how it differs from a VC fund or private equity firm, and why founders sell to one.
          ---

          [Back to Resources](/resources)
          Insights · Fundamentals

          # What Is a Private Venture Holding Company?

          The term sounds like finance jargon, but the model is one of the oldest and simplest in business: buy good companies, hold them, help them grow.

          A holding company is a business whose business is owning other businesses. It doesn't manufacture anything or serve customers directly; it acquires stakes in operating companies (often controlling ones) and holds them, usually with no plan to sell. Berkshire Hathaway is the famous version of the model. A *private venture* holding company like Vivid Partners applies the same idea at a smaller scale: acquiring and backing founder-led private businesses and holding them for the long term.

          ## How it differs from a VC fund

          Venture capital funds raise money from outside investors on a clock: typically a ten-year fund life. That clock shapes everything: they need each investment to have a shot at returning the whole fund, and they need exits. A holding company has no fund clock and no outside investors demanding liquidity on a schedule. It can own a good business for twenty years and never sell, which changes what kinds of businesses it can buy. A steady, profitable services company is a bad venture investment and a great holding company acquisition.

          ## How it differs from private equity

          Traditional private equity buys companies using significant debt, improves margins over roughly three to seven years, and sells: to another fund, a strategic acquirer, or the public markets. The model works, but it puts the company through a leveraged sprint and guarantees another change of ownership within a few years. Holding companies typically use little or no debt and don't need the resale, so the business isn't run for an exit. We've written a fuller comparison in [selling to a holding company vs. private equity](/holding-company-vs-private-equity).

          ## What the model means for a founder who sells

          Three practical differences show up at the negotiating table. First, continuity: because there's no planned resale, the team, brand, and customer relationships usually stay intact. The acquirer is buying the business as it is, not as raw material. Second, simpler deals: less leverage means fewer lender conditions and faster certainty. Third, alignment on horizon: a holding company doesn't need you to triple the business in four years; it needs the business to stay healthy for decades, which tends to match how founders already think.

          ## What holding companies look for

          Because the hold is long, the bar is durability rather than hypergrowth: businesses with real profits, customers who come back, and a reason they'll still matter in ten years. At Vivid Partners we focus on founder-led companies across services and technology: businesses where the model is proven and what's needed is capital, operating support, and patience rather than reinvention.

          ## The honest trade-offs

          No model is right for everyone. A strategic acquirer who desperately needs your product may pay more than anyone else. A venture round is the right tool if you want to raise capital and keep running a high-growth company rather than sell it. And holding companies are selective precisely because they can't exit mistakes easily. The model fits best when a founder wants a fair price, a fast clean process, and confidence that the company (and the people in it) will still be recognizable years later.

          This article is general information, not legal, tax, or financial advice. Talk to your lawyer and accountant before signing anything.

          **Thinking about your own next step?** Vivid Partners acquires and invests in founder-led businesses across the Greater Toronto Area and beyond. A conversation costs nothing and stays confidential.

          [Talk to us](/contact)
          `,
    },
    "/holding-company-vs-private-equity": {
          title: `Holding Company vs. Private Equity: What Founders Should Know · Vivid Partners`,
          markdown: `---
          title: Holding Company vs. Private Equity: What Founders Should Know · Vivid Partners
          url: https://www.vividpartners.ca/holding-company-vs-private-equity
          description: Comparing the two paths founder by founder: deal structure, debt, timelines, what happens to your team, and how to tell which buyer type fits your business.
          ---

          [Back to Resources](/resources)
          Insights · Selling

          # Selling to a Holding Company vs. Private Equity: What Founders Should Know

          Both will make you an offer. They are not offering the same future for your company. Here is how the two models actually differ once the deal closes.

          When a profitable founder-led business goes to market, the two most common financial buyers it meets are private equity funds and holding companies. The offers can look similar on paper. What happens in the five years after closing usually doesn't. Understanding the difference before you sign an LOI is worth more than another turn of EBITDA on the headline price.

          ## The clock is the difference

          Private equity funds have investors of their own who expect their money back, with returns, on a schedule, which means a PE buyer is planning its exit from your company before it buys it. Typically that's a three-to-seven-year hold, then a sale. A holding company has no fund timeline. It buys with the intention of owning indefinitely. Every other difference between the two models flows from this one.

          ## Debt, and who carries it

          The classic PE playbook is the leveraged buyout: a meaningful portion of your purchase price is borrowed, and the debt lands on the company's balance sheet. Serviced well, leverage amplifies returns; either way, it tightens the constraints the business operates under: capital expenditure, hiring, and inventory decisions all start answering to a lender covenant. Holding companies typically buy with little or no leverage, which leaves the business running roughly the way it did the day before the sale.

          ## What happens to your people

          PE ownership usually brings a value-creation plan: new reporting cadence, sometimes new executives, cost reviews, and add-on acquisitions to bulk up for exit. Some teams thrive under that; some cultures don't survive it. Holding companies vary too, but the structural incentive is different: when you're holding forever, the existing team *is* the plan, and burning them out has no offsetting payoff at exit. Ask any buyer for references from founders they've already acquired; the pattern in those calls is the real answer.

          ## Price, structure, and the second bite

          PE funds, competing in auctions with borrowed money, sometimes pay the highest headline number, often with earn-outs and rollover equity attached. Rolled equity gives you a "second bite" when the fund exits, which can be lucrative, but it means part of your price depends on a future sale you don't control. Holding company offers tend toward simpler structures (more certainty at close, fewer contingencies), occasionally at a somewhat lower headline. Which trade is right depends on whether you value maximum theoretical price or maximum certainty and continuity.

          ## Questions that sort buyers fast

          - How is this acquisition being financed, and how much debt goes on the company?
          - When do you expect to sell the business, and to whom?
          - What happened to the management teams of the last three companies you bought?
          - What changes in the first twelve months?
          - How much of my price is cash at closing versus contingent?

          Neither model is virtuous or villainous. They're different tools. If your goal is the biggest possible number and you're comfortable with the company being resold, a well-run PE process is a fine path. If your goal is a fair price, a fast close, and a business that still looks like itself in a decade, that's the profile a long-term holder like [Vivid Partners](/) is built for.

          This article is general information, not legal, tax, or financial advice. Talk to your lawyer and accountant before signing anything.

          **Thinking about your own next step?** Vivid Partners acquires and invests in founder-led businesses across the Greater Toronto Area and beyond. A conversation costs nothing and stays confidential.

          [Talk to us](/contact)
          `,
    },
    "/how-small-businesses-are-valued": {
          title: `How Small Businesses Are Actually Valued · Vivid Partners`,
          markdown: `---
          title: How Small Businesses Are Actually Valued · Vivid Partners
          url: https://www.vividpartners.ca/how-small-businesses-are-valued
          description: SDE, EBITDA, multiples, and the factors that actually move them: a working explanation of small business valuation in Canada from Vivid Partners.
          ---

          [Back to Resources](/resources)
          Insights · Valuation

          # How Small Businesses Are Actually Valued

          Forget the online calculators. Valuation is a short formula driven by a long list of judgment calls. Here is what the formula is and what moves it.

          Ask what your business is worth and you'll hear the same shape of answer from every serious buyer: a measure of normalized profit, times a multiple. The formula fits on a napkin. Everything interesting is in how the two numbers get set. Founders routinely leave money on the table by not understanding either one.

          ## Step one: normalized profit

          For owner-operated businesses, buyers use SDE (seller's discretionary earnings), which starts from net profit and adds back the owner's salary, personal expenses run through the company, one-time costs, and other discretionary items. For larger businesses with management in place, the measure is EBITDA, usually adjusted the same way. This "normalization" is a negotiation in itself: every add-back you claim needs a paper trail, and aggressive add-backs that collapse under diligence cost you credibility along with the dollars. Clean books a year before a sale are the cheapest valuation boost available.

          ## Step two: the multiple

          In the Canadian lower middle market, most businesses trade somewhere between two and six times SDE/EBITDA, with small service businesses clustered at the lower end and larger, more transferable companies earning more. The multiple is really a price on risk, specifically the risk that the profit stops when ownership changes. Things that push it up: revenue that recurs under contract, a customer base where no single account dominates, a management team that stays, documented processes, and growth that doesn't depend on the founder's personal relationships. Things that push it down: the founder being the top salesperson, one customer over 20-30% of revenue, declining revenue, and industries in structural decline.

          ## The founder-dependence discount

          This is the one that stings, because it punishes exactly the hustle that built the company. If customers buy because of you, if pricing lives in your head, if every employee reports to you directly, the buyer isn't buying a business, they're buying a job with your name on it, and they'll price it accordingly. The fix takes time, not money: put a second layer of management in place, write down how things work, and get yourself out of day-to-day delivery. Founders who do this eighteen months before selling often move their multiple more than any negotiation could.

          ## Why two honest buyers offer different numbers

          Valuation isn't a single truth; it's a price from a particular buyer's seat. A strategic acquirer who can fold your revenue into their existing overhead can afford more than the standalone math supports. A leveraged buyer is constrained by what lenders will finance. A long-term holder like [a holding company](/what-is-a-holding-company) prices durability (steady cash flow it can hold for decades) and cares comparatively more about structure and continuity than squeezing the last turn out of the multiple. Getting two or three offers isn't just leverage; it's information about what kind of asset you've built.

          ## What the number is not

          Your valuation is not what you need for retirement, not what a competitor sold for in a headline, and not revenue times a multiple you saw on Twitter. Anchoring to numbers like these is the most common reason good businesses sit unsold for years. The market price of a business, like anything else, is what a real buyer with real money will actually pay. Improving that number is a two-year project, not a negotiating tactic.

          This article is general information, not legal, tax, or financial advice. Talk to your lawyer and accountant before signing anything.

          **Thinking about your own next step?** Vivid Partners acquires and invests in founder-led businesses across the Greater Toronto Area and beyond. A conversation costs nothing and stays confidential.

          [Talk to us](/contact)
          `,
    },
    "/succession-planning-canada": {
          title: `Succession Planning for Canadian Business Owners · Vivid Partners`,
          markdown: `---
          title: Succession Planning for Canadian Business Owners · Vivid Partners
          url: https://www.vividpartners.ca/succession-planning-canada
          description: Most Canadian owners have no written succession plan. A practical guide to the options: family transfer, management buyout, or sale, and the timeline each one needs.
          ---

          [Back to Resources](/resources)
          Insights · Succession

          # Succession Planning for Canadian Business Owners

          A huge share of Canadian small businesses will change hands this decade, and most owners have no written plan. Here are the real options and what each one takes.

          Canada is in the middle of the largest ownership transition in its history: a majority of small-business owners are within sight of retirement, and industry surveys consistently find that only a small fraction have a formal succession plan. The result, repeated thousands of times a year, is a rushed sale from a weak position, or a business that simply closes, taking jobs and decades of built value with it. Succession planning is just deciding, while you still have time and leverage, which of a handful of doors you'll eventually walk through.

          ## Door one: family

          Passing the business to children is the default assumption and the least common outcome: most next generations, quite reasonably, have their own plans. Where family succession is real, start years early: the successor needs to actually run the business (not shadow you) well before the transfer, and the structure needs professional attention. Recent federal rule changes have made genuine intergenerational transfers more tax-workable in Canada, but the details are technical and evolving. This is squarely accountant territory.

          ## Door two: your own people

          A management buyout keeps the business in hands that already know it. The perennial problem is money: managers rarely have the capital to pay a fair price outright, so MBOs get financed with vendor take-backs (you effectively lend them the purchase price and get paid from future profits), bank debt, or an outside partner who backs the team. MBOs preserve culture better than almost any other path, but understand that a heavily vendor-financed deal means your retirement remains tied to the company's performance for years after you hand over the keys.

          ## Door three: sell the company

          An outside sale (to a competitor, a strategic acquirer, or a financial buyer such as a [holding company](/what-is-a-holding-company)) is how most successful exits actually happen. It usually produces the best price and the cleanest break, and buyers like Vivid Partners exist specifically for owners who want the business to continue as itself, with the team intact, after they step back. The trade-off is that a sale process takes real preparation; our guide to [selling a business in Ontario](/selling-your-business-in-ontario) covers that arc in detail.

          ## The timeline nobody wants to hear

          Whichever door you pick, the honest preparation window is two to five years. Cleaning up financials takes a year of statements. Reducing founder-dependence takes management hires and handover. Tax structuring (capital gains exemption qualification, estate freezes, family trusts) has multi-year clocks attached in some cases. Owners who start at 55 choose their exit; owners who start after a health scare at 68 take what's offered.

          ## A plan fits on one page

          A real succession plan is not a binder. It's a page that answers five questions: who takes over in each scenario (including the emergency one), what the business is roughly worth today and what you need it to be worth, which door you're aiming for, what has to be true for that door to open, and who your advisors are. Write it down, share it with your family and your accountant, and revisit it yearly. The owners who do this sell on their own terms; the ones who don't, sell on someone else's.

          This article is general information, not legal, tax, or financial advice. Talk to your lawyer and accountant before signing anything.

          **Thinking about your own next step?** Vivid Partners acquires and invests in founder-led businesses across the Greater Toronto Area and beyond. A conversation costs nothing and stays confidential.

          [Talk to us](/contact)
          `,
    },
};
